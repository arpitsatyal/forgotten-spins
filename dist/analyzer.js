...
class Analyzer {
    constructor(client, period = '12month') {
        this.client = client;
        this.period = period;
    }
    isCompilation(albumName) {
        const lower = albumName.toLowerCase();
        return (lower.includes('greatest hits') ||
    ...
    async getForgottenAlbum(genre) {
        ...
        const recentKeys = new Set(recentTop.map((a) => `${this.cleanString(a.artist)}:${this.cleanString(a.name)}`));
        // Filter overall list
        let candidates = overallTop.filter((album) => {
            ...
            if (this.isCompilation(album.name))
                return false;
            // 3. Must have significant play count (e.g. > 30 plays)
            if (album.playcount < 30)
                return false;
            return true;
        });
