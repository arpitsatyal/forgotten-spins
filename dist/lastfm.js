...
class LastFmClient {
    ...
    async fetch(method, params) {
        ...
        try {
            const response = await axios_1.default.get(`${API_ROOT}?${query.toString()}`);
    ...
    async getTopAlbums(period = 'overall', limit = 100) {
        const response = await this.fetch('user.gettopalbums', {
            period,
            limit,
        });
    ...
    async findLastPlayed(artist, albumName) {
        try {
            ...
            const response = await this.fetch('user.getartisttracks', {
                artist,
                limit: 200,
            });
    ...
    async getAlbumTags(artist, album) {
        try {
            const response = await this.fetch('album.getinfo', {
                artist,
                album,
            });
